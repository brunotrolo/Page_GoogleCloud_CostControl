...
const TOOL_REGISTRY: ToolDef[] = [
  ...
  },
  {
    ...
    description: "Calcular IV Rank de vários ativos de uma vez e ranquear por IV Rank decrescente. Sem o parâmetro 'tickers', usa a whitelist padrão de 28 ativos líquidos (espelho da aba DADOS_ATIVOS; sincronizada dinamicamente se DADOS_ATIVOS_CSV_URL estiver configurada, senão lista fixa). Usa cache de 4h (tickers em cache não consomem chamadas) e processa os demais em lotes de 3 com 300ms entre lotes para evitar rate limit (HTTP 429). Retorna números brutos por ativo (iv_rank, iv_rank_63d, iv_rank_252d, historico_insuficiente, alerta_evento) — SEM classificação/triagem pronta embutida; quem chama decide o que é 'pronto para operar'.",
    properties: {
      tickers: { type: "array",   description: "Lista de códigos de ações (ex: [\"VALE3\",\"PETR4\"]). Se omitido, usa a whitelist padrão de 28 ativos (aba DADOS_ATIVOS).", items: { type: "string" } },
      periodo: { type: "integer", description: "Janela em dias úteis para o IV Rank. Padrão: 252. Aceita: 21, 63, 126, 252" },
    },
    required: [],
  ...
  // ── Plano mensal de travas Bull Put Spread ──────────────────────────────────
  {
    ...
    description:
      "Ferramenta ANALÍTICA (apenas calcula — não envia ordens). Aplica filtros de qualidade (IV Rank >= 50, tendência M9/M21 >= 1.0, volume de PUT >= R$5M) e seleciona a trava real da cadeia ao vivo (delta/bid/ask reais) para cada ativo testado, dimensionando lotes por candidato aprovado. Retorna 'ativos_avaliados': TODO ativo testado (aprovado ou não) num único array, cada um com seus números crus (spot, iv_rank, m9m21, volume_put_mm) e, se eliminado, o 'motivo' factual (ex.: 'IV Rank 63d=43%'); a ferramenta NÃO separa em duas listas por julgamento nem decide 'meta atingida' — isso é da skill, comparando viabilidade.premium_projetado com parametros.meta_mensal. 'plano_execucao' traz a trava dimensionada por candidato aprovado (dados de execução, não recomendação). Sem 'tickers', usa a whitelist padrão de 28 ativos (espelho da aba DADOS_ATIVOS, mesma fonte de get_iv_rank_bulk/get_smart_money_tracker/get_backtest_estrutural — sincronizada dinamicamente se DADOS_ATIVOS_CSV_URL estiver configurada, senão lista fixa). DICA DE USO: se muitos ativos saírem eliminados com motivo 'IV Rank ... < 50%', chame de novo passando iv_rank_periodo=252 — a janela de 63d (padrão) reage ao regime recente e pode reprovar ativos com IV Rank alto na faixa anual. Se o motivo for 'sem trava com prêmio/liquidez suficiente', o gargalo é o prêmio da cadeia (não o IV Rank): mudar o período não ajuda.",
    properties: {
      ...
      tickers:        { type: "array",   description: "Lista de ativos a avaliar. Se omitido, usa a whitelist padrão de 28 ativos (aba DADOS_ATIVOS).", items: { type: "string" } },
    },
    required: ["capital"],
  ...
  // ── Smart Money Tracker — Whale & Block Trade Tracker ───────────────────────
  {
    ...
    description:
      "Rastreia fluxo institucional anômalo na cadeia de opções (Whale & Block Trade Tracker). Caça opções de curto prazo com alto volume financeiro no dia cujo TICKET MÉDIO por negócio é de porte institucional (varejo fracionado não sustenta ticket médio alto). Processa em lotes de 3 com 300ms (evita HTTP 429) e ordena por volume financeiro decrescente. Sem 'tickers', varre a whitelist padrão de 28 ativos (espelho da aba DADOS_ATIVOS). OBS: a OpLab não expõe Open Interest nem gregas na chain — por isso o tracker usa volume/trades/financeiro (dados reais) e retorna delta=null.",
    properties: {
      tickers:                     { type: "array",   description: "Lista de ativos-base a varrer (ex: [\"PETR4\",\"VALE3\"]). Se omitido, usa a whitelist padrão de 28 ativos (aba DADOS_ATIVOS).", items: { type: "string" } },
  ...
  },
  {
    ...
    properties: {
      tickers:        { type: "array",   description: "Lista de códigos (ex: [\"VALE3\",\"PSSA3\"]). Se omitido, usa a whitelist padrão de 28 ativos (aba DADOS_ATIVOS)." },
