...
const TOOL_REGISTRY: ToolDef[] = [
  ...
  // ── Plano mensal de travas Bull Put Spread ──────────────────────────────────
  {
    ...
    description:
      "Ferramenta ANALÍTICA (apenas calcula — não envia ordens). Aplica filtros de qualidade (IV Rank >= 50, tendência M9/M21 >= 1.0, volume de PUT >= R$5M) e seleciona a trava real da cadeia ao vivo (delta/bid/ask reais) para cada ativo testado, dimensionando lotes por candidato aprovado. Retorna 'ativos_avaliados': TODO ativo testado (aprovado ou não) num único array, cada um com seus números crus (spot, iv_rank, m9m21, volume_put_mm) e, se eliminado, o 'motivo' factual (ex.: 'IV Rank 63d=43%'); a ferramenta NÃO separa em duas listas por julgamento nem decide 'meta atingida' — isso é da skill, comparando viabilidade.premium_projetado com parametros.meta_mensal. 'plano_execucao' traz a trava dimensionada por candidato aprovado (dados de execução, não recomendação). Sem 'tickers', usa a whitelist padrão de 26 ativos (espelho da aba DADOS_ATIVOS, mesma fonte de get_iv_rank_bulk/get_smart_money_tracker/get_backtest_estrutural — sincronizada dinamicamente se DADOS_ATIVOS_CSV_URL estiver configurada, senão lista fixa). DICA DE USO: se muitos ativos saírem eliminados com motivo 'IV Rank ... < 50%', chame de novo passando iv_rank_periodo=252 — a janela de 63d (padrão) reage ao regime recente e pode reprovar ativos com IV Rank alto na faixa anual. Se o motivo for 'sem trava com prêmio/liquidez suficiente', o gargalo é o prêmio da cadeia (não o IV Rank): mudar o período não ajuda.",
    properties: {
      ...
      tickers:        { type: "array",   description: "Lista de ativos a avaliar. Se omitido, usa a whitelist padrão de 26 ativos (aba DADOS_ATIVOS).", items: { type: "string" } },
    },
    required: ["capital"],
